(() => {
    ...
    function manipulateHeader() {
        ...
        const firstSlideIsActive = () => {
            const activeSection = document.querySelector(".swiper-slide-active");
            let sectionIsActive;
            sections.forEach(((section, index) => {
                index === 0 && section === activeSection ? sectionIsActive = true : 0;
            }));
            return sectionIsActive;
        };
        function paintHeader() {
            header.style.backgroundColor = "#E7F066";
        }
        function makeTransparentHeader() {
            header.style.backgroundColor = "transparent";
        }
        function addActiveClass() {
            !document.documentElement.classList.contains("_header-visible") ? document.documentElement.classList.add("_header-visible") : 0;
        }
        function removeActiveClass() {
            document.documentElement.classList.contains("_header-visible") ? document.documentElement.classList.remove("_header-visible") : 0;
        }
        function mobileScroll() {
            ...
            function getScrollDirection() {
                ...
                document.addEventListener("touchstart", (e => {
                    startPosition = e.changedTouches[0].screenY;
                }));
                document.addEventListener("touchend", (e => {
                    endPosition = e.changedTouches[0].screenY;
                    if (endPosition - startPosition > 30) {
                        scrollDirection = "up";
                        scrollUp();
                    } else if (endPosition - startPosition < -30) {
        ...
        document.addEventListener("mousewheel", (e => {
            const isFirstSlide = firstSlideIsActive();
            if (e.deltaY > 0) removeActiveClass(); else if (e.deltaY < 0 && isFirstSlide) {
                addActiveClass();
                makeTransparentHeader();
            }
        }));
        document.addEventListener("mousemove", (e => {
            const isFirstSlide = firstSlideIsActive();
            if (e.clientY < headerHeight / 2) addActiveClass();
            if (isFirstSlide) makeTransparentHeader(); else paintHeader();
        }));
        header.addEventListener("mouseleave", (e => {
            document.querySelectorAll(".section");
            const isFirstSlide = firstSlideIsActive();
            if (!isFirstSlide) removeActiveClass();
        }));
        mobileScroll();
    }
    function manipulateFooter() {
        document.querySelector("._footer-open");
        const footer = document.querySelector(".footer");
        const htmlElement = document.documentElement;
        function footerOpen() {
            !htmlElement.classList.contains("_footer-visible") ? htmlElement.classList.add("_footer-visible") : 0;
        }
        function footerClose() {
            htmlElement.classList.contains("_footer-visible") ? htmlElement.classList.remove("_footer-visible") : 0;
        }
        document.addEventListener("click", (e => {
            if (e.target.closest("._footer-open")) footerOpen(); else if (!e.target.closest(".footer")) footerClose();
        }));
        footer.addEventListener("mouseleave", (e => {
            footerClose();
        }));
        document.addEventListener("mousewheel", (e => {
            e.deltaY < 0 && footerClose();
        }));
    }
    manipulateHeader();
    manipulateFooter();
